...
x_mi_train = Get_Data("dataset\mf_mi_train.dat", 0, 1)
y_mi_train = Get_Data("dataset\mf_mi_train.dat", 1, 2)
...
plt.plot(x_lo_train, y_lo_train, label='Low fidelity train')
plt.plot(x_test, y_predict[:,0], label='Low fidelity predict', linestyle='dashed')

plt.plot(x_mi_train, y_mi_train, label='Middel fidelity train')
plt.plot(x_test, y_predict[:,1], label='Middel fidelity predict', linestyle='dashed')
